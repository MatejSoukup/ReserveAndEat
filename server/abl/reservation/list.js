...
async function listAbl(req , res){
    try {
        ...
        if(filters.userId){
            ...
            reservationList = reservationList.filter(item => item.userId === filters.userId)

        }else if(filters.restaurantId){    
            
            reservationList = reservationList.filter(item => item.restaurantId === filters.restaurantId)
        }

        //Maping user and restaurant
        reservationList = await Promise.all(reservationList.map(async reservation => {
            reservation.user = await daoUser.get(reservation.userId);
            reservation.restaurant = await daoRestaurant.get(reservation.restaurantId);
                return reservation; 
        }));
